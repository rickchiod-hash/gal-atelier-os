...
$conflictos = git diff --name-only --diff-filter=U 2>$null
if ($conflictos) {
...
gh pr create --base release --head develop --title "Promote to Release (Homologacao)" --body "Automated PR from develop to release branch for homologation testing."
$releasePR = gh pr list --base release --head develop --json number --jq '.[0].number'
Write-Host "✅ PR #$releasePR criado (develop -> release)!" -ForegroundColor Green

# 6. Aguardar checks do release e fazer merge
Write-Host "`n[6/7] Aguardando checks do release..." -ForegroundColor Yellow
Start-Sleep -Seconds 60
gh pr merge $releasePR --squash 2>&1 | Out-Null
...
gh pr create --base main --head release --title "Promote to Main (Production)" --body "Automated PR from release to main branch for production deployment."
$mainPR = gh pr list --base main --head release --json number --jq '.[0].number'
Write-Host "✅ PR #$mainPR criado (release -> main)!" -ForegroundColor Green
...
Write-Host ("  - PR #{0}: develop -> release (MERGED)" -f $releasePR)
Write-Host ("  - PR #{0}: release -> main (PENDING MERGE)" -f $mainPR)
